...
ax[0,0].annotate("Pd\n10 mM potassium formate", (0.02, 0.85), xycoords="axes fraction", fontsize=24, fontweight="bold") #add the catalyst label
...
ax[0, 2].annotate("Pd\n500 mM potassium formate", (0.02, 0.85), xycoords="axes fraction", fontsize=24, fontweight="bold") #add the catalyst label

df= DF.query("catalyst == 'Pd' & temperature_C == 75 & formate_mM == 1000 & stock_batch_number >= 0") #filter the data
ax[0, 3].errorbar(df.IPA_molefrac, df.EP_generation_rate, yerr=df.EP_generation_rate_std_err, c="k", capsize=4, linewidth=1, zorder=0) #plot the standard error

df[df["IPA_molefrac"] == 0.0].plot.scatter("IPA_molefrac", "EP_generation_rate", ax=ax[0, 3], label="0% IPA",s=60, c="b", edgecolor="k") #plot the 0% IPA data
df[df["IPA_molefrac"] == 0.1].plot.scatter("IPA_molefrac", "EP_generation_rate", ax=ax[0, 3], label="10% IPA",s=60, c="#58a8f9", edgecolor="k") #plot the 10% IPA data
df[df["IPA_molefrac"] == 0.2].plot.scatter("IPA_molefrac", "EP_generation_rate", ax=ax[0, 3], label="20% IPA",s=60, c="g",edgecolor="k") #plot the 20% IPA data
...
df= DF.query("catalyst == 'Pd' & temperature_C == 75 & formate_mM == 500 & stock_batch_number >= 0") #filter the data
ax[1, 2].errorbar(df.IPA_molefrac, df.EP_generation_rate, yerr=df.EP_generation_rate_std_err, c="k", capsize=4, linewidth=1, zorder=0) #plot the standard error

df[df["IPA_molefrac"] == 0.0].plot.scatter("IPA_molefrac", "EP_generation_rate", ax=ax[1, 2], label="0% IPA",s=60, c="b", edgecolor="k") #plot the 0% IPA data
df[df["IPA_molefrac"] == 0.1].plot.scatter("IPA_molefrac", "EP_generation_rate", ax=ax[1, 2], label="10% IPA",s=60, c="#58a8f9", edgecolor="k") #plot the 10% IPA data
df[df["IPA_molefrac"] == 0.2].plot.scatter("IPA_molefrac", "EP_generation_rate", ax=ax[1, 2], label="20% IPA",s=60, c="g",edgecolor="k") #plot the 20% IPA data
ax[1, 2].annotate("Pt\n500 mM potassium formate", (0.02, 0.85), xycoords="axes fraction", fontsize=24, fontweight="bold") #add the catalyst label
...
for row in ax:
    for axi in row:
        axi.set_xlabel("IPA Concentration (mole/mole)", labelpad=15, fontsize=26) #add x axis label
        axi.set_ylabel("Ethylphenol Generation Rate (mM/g s)", labelpad=15, fontsize=26) #add y axis label
        ...
        axi.set_ylim(-0.05, 10) #set the y axis limits
